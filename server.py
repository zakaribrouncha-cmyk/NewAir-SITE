...
DID_COOKIE = 'newair_discord_id'
USER_SESSIONS = {}
ADMIN_SESSIONS = {}
OAUTH_STATES = {}
CUSTOM_JS = "/assets/newair-custom.js?v=force-ui-7"
...
def ready(need_roles=False):
    c = cfg()
    return bool(c['cid'] and c['sec'] and c['bt'] and c['gid'] and (c['all'] or not need_roles))
def rjson(method, url, headers=None, **kw):
    h = {'User-Agent': 'Mozilla/5.0 NewAirSite/1.0', 'Accept': 'application/json'}
...
def exchange(code, redir):
    c = cfg()
    return rjson('POST', 'https://discord.com/api/oauth2/token', {'Content-Type': 'application/x-www-form-urlencoded'}, data={'client_id': c['cid'], 'client_secret': c['sec'], 'grant_type': 'authorization_code', 'code': code, 'redirect_uri': redir})
def get_user(at): return rjson('GET', 'https://discord.com/api/users/@me', {AUTH: 'Bearer ' + at})
def get_member(uid):
    c = cfg(); return rjson('GET', f"https://discord.com/api/guilds/{c['gid']}/members/{uid}", {AUTH: 'Bot ' + c['bt']})
def get_member_oauth(at):
    c = cfg(); return rjson('GET', f"https://discord.com/api/users/@me/guilds/{c['gid']}/member", {AUTH: 'Bearer ' + at})
...
def role(uid, rid, method):
    ...
    c = cfg()
    try:
        rjson(method, f"https://discord.com/api/guilds/{c['gid']}/members/{uid}/roles/{rid}", {AUTH: 'Bot ' + c['bt']})
        return True
    except Exception as e:
        print('role error', e); return False
def set_status(uid, status):
    rows = read_json(USERS, [])
    item = next((x for x in rows if str(x.get('discord_id')) == str(uid)), None)
    now = time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())
    if item:
        item['status'] = status
        ...
        write_json(USERS, rows)
    if status == 'accepted':
        role(uid, REFUSED_ROLE_ID, 'DELETE'); role(uid, ACCEPTED_ROLE_ID, 'PUT')
    if status == 'rejected':
        role(uid, ACCEPTED_ROLE_ID, 'DELETE'); role(uid, REFUSED_ROLE_ID, 'PUT')
    return item
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def inject_html(self, html):
        script = f"<script src='{CUSTOM_JS}' defer data-newair-force='1'></script>"
        html = html.replace("<script src='/assets/newair-custom.js?v=roles3' defer></script>", '')
        html = html.replace('<script src="/assets/newair-custom.js?v=roles3" defer></script>', '')
        html = html.replace("<script src='/assets/newair-custom.js?v=newdomain1' defer></script>", '')
        html = html.replace('<script src="/assets/newair-custom.js?v=newdomain1" defer></script>', '')
        if CUSTOM_JS not in html:
    ...
    def redir(self):
        old = 'newair-site.onrender.com'
        env_redir = cfg()['redir']
        if env_redir and old not in env_redir: return env_redir
        return self.base() + '/api/discord/callback' if 'newair-site-3hnq.onrender.com' in self.base() else NEW_BASE + '/api/discord/callback'
    def cookie(self, n):
    ...
    def do_GET(self):
        ...
        if path == '/api/discord/debug':
            c = cfg(); return self.send_json({'ok': True, 'redirect_uri_used': self.redir(), 'current_base': self.base(), 'custom_js': CUSTOM_JS, 'html_inject': True, 'client_id_set': bool(c['cid']), 'bot_token_set': bool(c['bt']), 'guild_id_set': bool(c['gid']), 'admin_roles_count': len(c['all']), 'staff_roles_count': len(c['staff'])})
        ...
        if path == '/api/discord/callback':
            ...
            try:
                ...
                try: mem = get_member(du['id'])
                    ...
                    except Exception as e2: print('member roles error', e2)
                ...
                return self.send_html(f"Connexion Discord impossible.<br><br>Erreur exacte : <pre>{esc(e)}</pre><br>Redirect utilisé : <code>{esc(saved.get('redirect_uri') or self.redir())}</code>", 500)
