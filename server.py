...
USER_COOKIE='newair_user_session'; ADMIN_COOKIE='newair_admin_session'; DID_COOKIE='newair_discord_id'
USER_SESSIONS={}; ADMIN_SESSIONS={}; OAUTH_STATES={}
NEW_BASE='https://newair-site-3hnq.onrender.com'
CUSTOM_JS='/assets/newair-custom.js?v=authfix2'
...
def env_list(*names):
    out=[]
    for n in names:
        out += [clean(x) for x in os.environ.get(n,'').split(',') if clean(x)]
...
def cfg():
    admin=env_list('DISCORD_ADMIN_ROLE_IDS','ID_ROLE_ADMIN','ADMIN_ROLE_ID','ROLE_ADMIN')
    staff=env_list('DISCORD_STAFF_ROLE_IDS','ID_ROLE_STAFF','STAFF_ROLE_ID','ROLE_STAFF')
    superadmin=env_list('DISCORD_SUPERADMIN_ROLE_IDS','ID_ROLE_SUPERADMIN','SUPERADMIN_ROLE_ID','ROLE_SUPERADMIN')
    fondateur=env_list('DISCORD_FONDATEUR_ROLE_IDS','ID_ROLE_FONDATEUR','FONDATEUR_ROLE_ID','ROLE_FONDATEUR','ID_ROLE_PROPRIETAIRE')
    haut=env_list('DISCORD_HAUT_GRADE_PANEL_ROLE_IDS','ID_ROLE_HAUT_GRADE_PANEL','HAUT_GRADE_PANEL_ROLE_ID','ROLE_HAUT_GRADE_PANEL')
...
def ready_login():
    c=cfg(); return bool(c['cid'] and c['sec'])
def ready_admin():
    c=cfg(); return bool(c['cid'] and c['sec'] and c['gid'] and c['all'])
def h(extra=None):
...
def rjson(method,url,headers=None,**kw):
    ...
    if r.status_code>=400:
        ...
        raise RuntimeError(f'HTTP {r.status_code}: {d}')
...
def grade_from_roles(roles):
    c=cfg(); roles=set(map(str,roles or []))
...
def exchange(code,redir):
    c=cfg(); return rjson('POST','https://discord.com/api/oauth2/token',{'Content-Type':'application/x-www-form-urlencoded'},data={'client_id':c['cid'],'client_secret':c['sec'],'grant_type':'authorization_code','code':code,'redirect_uri':redir})
def get_user(at): return rjson('GET','https://discord.com/api/users/@me',{AUTH:'Bearer '+at})
def get_member(uid):
    c=cfg()
    if not c['bt'] or not c['gid']: return None
    return rjson('GET',f"https://discord.com/api/guilds/{c['gid']}/members/{uid}",{AUTH:'Bot '+c['bt']})
def get_member_oauth(at):
    c=cfg()
    if not c['gid']: return None
    return rjson('GET',f"https://discord.com/api/users/@me/guilds/{c['gid']}/member",{AUTH:'Bearer '+at})
def upsert_user(u,member=None):
    rows=read_json(USERS,[]); now=time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime())
    item=next((x for x in rows if str(x.get('discord_id'))==str(u.get('id'))),None)
...
def role(uid,rid,method):
    ...
    c=cfg()
    if not c['bt'] or not c['gid']: return False
    try: rjson(method,f"https://discord.com/api/guilds/{c['gid']}/members/{uid}/roles/{rid}",{AUTH:'Bot '+c['bt']}); return True
...
def set_status(uid,status):
    ...
    if item: item['status']=status; item['reviewed_at']=now; write_json(USERS,rows)
    if status=='accepted': role(uid,REFUSED_ROLE_ID,'DELETE'); role(uid,ACCEPTED_ROLE_ID,'PUT')
    if status in ('rejected','refused'): role(uid,ACCEPTED_ROLE_ID,'DELETE'); role(uid,REFUSED_ROLE_ID,'PUT')
    return item
def esc(v): return str(v or '').replace('&','&amp;').replace('<','&lt;').replace('>','&gt;').replace('"','&quot;')
def team_data():
    out=[]; seen=set(); err=None; c=cfg()
    if c['bt'] and c['gid']:
        try:
            data=rjson('GET',f"https://discord.com/api/guilds/{c['gid']}/members?limit=1000",{AUTH:'Bot '+c['bt']})
            for m in data:
                gr=grade_from_roles(m.get('roles',[]))
                if not gr: continue
                u=m.get('user',{}); uid=str(u.get('id')); seen.add(uid)
                out.append({'name':m.get('nick') or u.get('global_name') or u.get('username') or 'Staff NewAir','role':gr,'discord_id':uid,'avatar':avatar(u),'subtitle':'Rôle Discord : '+gr})
        except Exception as e: err=str(e); print('team error',e)
    for u in read_json(USERS,[]):
        gr=u.get('grade') or grade_from_roles(u.get('roles',[])); uid=str(u.get('discord_id'))
...
def account_html(u):
    n=esc(u.get('global_name') or u.get('username') or 'Compte'); av=esc(u.get('avatar') or '/assets/newair-logo-swirl.png'); gr=esc(u.get('grade') or 'Joueur')
    admin="<a style='color:white' href='/admin/'>Admin Panel</a> · " if u.get('is_admin') else ''
    return f"""<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Compte NewAir</title><link rel='stylesheet' href='/assets/styles-Db9UqP69.css'><link rel='stylesheet' href='/assets/newair-custom.css?v=authfix2'></head><body style='margin:0;background:#000;color:white;font-family:Arial'><main style='max-width:900px;margin:0 auto;padding:100px 22px'><h1 style='font-size:54px'>MON COMPTE</h1><section style='border:1px solid #1e4f8f;border-radius:18px;padding:30px;background:#050b16'><img src='{av}' style='width:82px;height:82px;border-radius:999px;object-fit:cover'><h2>{n}</h2><p>Grade Discord : <b>{gr}</b></p>{admin}<a style='color:white' href='/equipe/'>Équipe</a> · <a style='color:white' href='/'>Accueil</a> · <a style='color:white' href='/api/discord/logout'>Déconnexion</a></section></main><script src='{CUSTOM_JS}' defer></script></body></html>"""
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def send_json(self,v,status=200):
        b=json.dumps(v,ensure_ascii=False).encode('utf-8'); self.send_response(status); self.send_header('Content-Type','application/json; charset=utf-8'); self.send_header('Cache-Control','no-store'); self.send_header('Content-Length',str(len(b))); self.end_headers(); self.wfile.write(b)
    def send_html(self,html,status=200):
        b=html.encode('utf-8'); self.send_response(status); self.send_header('Content-Type','text/html; charset=utf-8'); self.send_header('Cache-Control','no-store'); self.send_header('Content-Length',str(len(b))); self.end_headers(); self.wfile.write(b)
    def inject_html(self,html):
        css="<link rel='stylesheet' href='/assets/newair-custom.css?v=authfix2'>"; js=f"<script src='{CUSTOM_JS}' defer data-newair-force='1'></script>"
        if '/assets/newair-custom.css' not in html: html=html.replace('</head>',css+'</head>') if '</head>' in html else css+html
        for old in ("/assets/newair-custom.js?v=roles3","/assets/newair-custom.js?v=force-ui-7","/assets/newair-custom.js?v=newdomain1"):
            html=html.replace(f"<script src='{old}' defer></script>", '').replace(f'<script src="{old}" defer></script>', '')
        if '/assets/newair-custom.js' not in html: html=html.replace('</body>',js+'</body>') if '</body>' in html else html+js
        return html
    ...
    def redirect(self,loc,cookie=None):
        ...
        if cookie:
            for c in cookie if isinstance(cookie,(list,tuple)) else [cookie]: self.send_header('Set-Cookie',c)
        self.end_headers()
    def base(self):
    ...
    def redir(self):
        base=self.base(); er=cfg()['redir']
        if 'newair-site-3hnq.onrender.com' in base: return base+'/api/discord/callback'
        if er and 'newair-site-3hnq.onrender.com' in er: return er
        return NEW_BASE+'/api/discord/callback'
    def cookie(self,n):
    ...
    def current_user(self):
        ...
        u=find_user(self.cookie(DID_COOKIE)); return {'user':u,'expires':time.time()+31536000} if u else None
    def current_admin(self):
        s=ADMIN_SESSIONS.get(self.cookie(ADMIN_COOKIE) or '')
        if s and s.get('expires',0)>time.time(): return s
        u=self.current_user(); return {'admin':u['user'],'expires':time.time()+31536000} if u and u.get('user') and u['user'].get('is_admin') else None
    def body_json(self):
    ...
    def do_GET(self):
        ...
        if path=='/api/discord/debug':
            c=cfg(); return self.send_json({'ok':True,'redirect_uri_used':self.redir(),'current_base':self.base(),'custom_js':CUSTOM_JS,'client_id_set':bool(c['cid']),'client_secret_set':bool(c['sec']),'bot_token_set':bool(c['bt']),'guild_id_set':bool(c['gid']),'login_ready':ready_login(),'admin_ready':ready_admin(),'admin_roles_count':len(c['all']),'staff_roles_count':len(c['staff'])})
        ...
        if path=='/api/admin/me':
            s=self.current_admin(); return self.send_json({'ok':True,'admin':s['admin']}) if s else self.send_json({'ok':False,'configured':ready_admin()},401)
        if path=='/api/users':
        ...
        if path=='/api/discord/login':
            if not ready_login(): return self.send_json({'ok':False,'error':'DISCORD_CLIENT_ID ou DISCORD_CLIENT_SECRET manque sur Render'},500)
            mode=(q.get('mode') or ['user'])[0]; nxt=(q.get('next') or (['/admin/'] if mode=='admin' else ['/']))[0]; state=secrets.token_urlsafe(24); red=self.redir(); OAUTH_STATES[state]={'expires':time.time()+600,'mode':mode,'next':nxt,'redirect_uri':red}; c=cfg()
        ...
        if path=='/api/discord/callback':
            code=(q.get('code') or [''])[0]; state=(q.get('state') or [''])[0]; saved=OAUTH_STATES.pop(state,None) or {'mode':'user','next':'/','redirect_uri':self.redir()}
            if not code: return self.redirect('/')
            try:
                token=exchange(code,saved.get('redirect_uri') or self.redir()); du=get_user(token['access_token']); mem=None
                try: mem=get_member(du['id'])
                except Exception as e: print('bot member error',e)
                if not mem:
                    try: mem=get_member_oauth(token['access_token'])
                    except Exception as e: print('oauth member error',e)
                ...
                cs=[f'{USER_COOKIE}={sid}; Path=/; HttpOnly; SameSite=Lax; Max-Age=31536000',f'{DID_COOKIE}={item.get("discord_id")}; Path=/; SameSite=Lax; Max-Age=31536000']
                if item.get('is_admin'):
                    aid=secrets.token_urlsafe(32); ADMIN_SESSIONS[aid]={'expires':time.time()+31536000,'admin':item}; cs.append(f'{ADMIN_COOKIE}={aid}; Path=/; HttpOnly; SameSite=Lax; Max-Age=31536000')
                return self.redirect(saved.get('next') or '/',cs)
            except Exception as e: return self.send_html(f"Connexion Discord impossible.<br><pre>{esc(e)}</pre><br>Redirect utilisé : <code>{esc(saved.get('redirect_uri') or self.redir())}</code>",500)
...
if __name__=='__main__':
    os.chdir(ROOT); port=int(os.environ.get('PORT','4174')); print(f'NewAir web service on port {port}'); ThreadingHTTPServer(('0.0.0.0',port),Handler).serve_forever()
